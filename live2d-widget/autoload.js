...
(async () => {
  ...
  initWidget({
    ...
    // 模型 API：二选一，cdnPath 走静态 CDN（推荐，稳定），apiPath 走后端接口
    // 本地模型库（已部署 models/ 和 model_list.json）：
  ...
  fetch(live2d_path + 'waifu-tips.json')
    .then(r => r.json())
    .then(data => {
      if (data && data.message && Array.isArray(data.message.tapBody)) {
        tapMessages = data.message.tapBody;
      }
    })
    .catch(() => {
      tapMessages = ['你好呀～', '快来逗我玩吧！', '今天也要开心哦～', '点我干嘛呀？'];
    });

  const showTapMessage = () => {
    if (!tapMessages.length) return;
    let tipsEl = document.getElementById('waifu-tips');
    if (!tipsEl) {
      const waifuEl = document.getElementById('waifu');
      if (!waifuEl) return;
      tipsEl = document.createElement('div');
      tipsEl.id = 'waifu-tips';
      waifuEl.insertBefore(tipsEl, waifuEl.firstChild);
    }
  ...
  // 绑定点击事件到最外层 #waifu 容器（确保整个看板娘区域都能响应）
  let tapBound = false;
  const bindTap = () => {
    if (tapBound) return true;
    const waifuEl = document.getElementById('waifu');
    if (!waifuEl) return false;
    const onTap = (ev) => {
      showTapMessage();
    };
    waifuEl.addEventListener('pointerdown', onTap);
    waifuEl.addEventListener('touchstart', onTap, { passive: true });
    tapBound = true;
    return true;
  };
  let tries = 0;
  const timer = setInterval(() => {
    if (bindTap() || ++tries > 20) clearInterval(timer);
  }, 500);
})();
